...
// CRO processing status codes
const CRO_STATUS_CODES = [
  { code: 'D01', description: 'Cleared / Release Order Issued' },
  { code: 'P01', description: 'Under Examination / Pending Inspection' },
  { code: 'P02', description: 'Pending Document Verification' },
  { code: 'R01', description: 'Rejected / Refer to CFA' },
  { code: 'H01', description: 'Held / Awaiting Scanning' },
];

// --- Utility formatters ---

export function fakeTancisRefNumber() {
  const prefix = String(faker.number.int({ min: 100000000, max: 999999999 }));
  const year = new Date().getFullYear();
  const seq = String(faker.number.int({ min: 1, max: 9999999 })).padStart(7, '0');
  return `${prefix}${year}TZAR${seq}`;
}
...
export function generateCro(statusCode = null) {
  const now = new Date();
  const status = statusCode
    ? CRO_STATUS_CODES.find(s => s.code === statusCode) ?? CRO_STATUS_CODES[0]
    : faker.helpers.arrayElement(CRO_STATUS_CODES);

  return {
    header: {
      interface_id: 'IF-E-PTL-001',
      send_date_and_time: formatSendDateTime(now),
      sender_id: 'TANCIS',
      receiver_id: 'PCS',
    },
    message_info: {
      processing_status_notice_date_and_time: formatSendDateTime(now).slice(0, 14),
      notice_customs_code: 'TZDL',
      tancis_reference_number: fakeTancisRefNumber(),
      processing_status_code: status.code,
      processing_status_description: status.description,
    },
  };
}
